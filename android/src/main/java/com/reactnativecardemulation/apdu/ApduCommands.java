...
public class ApduCommands {
   ...
   */
  public static final byte[] MASTERCARD_MSD_SELECT = {
    (byte) 0x00,  // CLA
    (byte) 0xA4,  // INS
    (byte) 0x04,  // P1
    (byte) 0x00,  // P2
  ...
  public static final byte[] MASTERCARD_MSD_SELECT_RESPONSE = {
    (byte) 0x6F,  // File Control Information (FCI) Template
    (byte) 0x17,  // length = 30 (0x1E)
  ...
  public static final byte[] COMPUTE_CRYPTOGRAPHIC_CHECKSUM_COMMAND = {
    (byte) 0x80,
    (byte) 0x2A,
    (byte) 0x8E,
    (byte) 0x80
  ...
  public static boolean isComputeCryptographicChecksumCommand(byte[] apdu) {
    return (apdu.length > 4 &&
      apdu[0] == COMPUTE_CRYPTOGRAPHIC_CHECKSUM_COMMAND[0] &&
      apdu[1] == COMPUTE_CRYPTOGRAPHIC_CHECKSUM_COMMAND[1] &&
      apdu[2] == COMPUTE_CRYPTOGRAPHIC_CHECKSUM_COMMAND[2] &&
      apdu[3] == COMPUTE_CRYPTOGRAPHIC_CHECKSUM_COMMAND[3]
    );
  }
}
